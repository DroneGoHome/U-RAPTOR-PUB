...
from processing import load_dat_file, length_corrector, get_spectrogram, save_spectrogram, get_stft_spectrogram, get_power_spectrogram_db, get_color_img, add_awgn_noise, save_color_img
...
def get_starting_index(folder_path: str, pattern: str) -> int:
	...
	try:
		...
		file_path_to_check = os.path.join(folder_path, f"{pattern}_{index}.pkl") # Check against a common extension like .pkl

		# Loop while a file with the current index exists
		# Note: This checks for the base name without extension. A more robust check
		# might involve globbing or checking for specific file extensions if needed.
		# For simplicity, we check for a common extension like .pkl as an example.
		# If multiple file types (.pkl, .png) use the same index, checking one is usually sufficient.
		while os.path.exists(file_path_to_check):
			index += 1 # Increment the index to check the next number
			# Update the file path for the next iteration
			file_path_to_check = os.path.join(folder_path, f"{pattern}_{index}.pkl")
...
def get_drone_center_freq(file_path: str) -> Optional[int]:
	"""
	Extracts the drone center frequency from the filename and converts it to an integer.
	...
	Returns:
		The extracted drone center frequency as an integer, or None if parsing fails
		(e.g., filename doesn't contain enough '_' separators, or the frequency part is not a valid integer).
	"""
	try:
		# Get the base name of the file
		base_name = os.path.basename(file_path)
		...
		if len(parts) > 3:
			# Attempt to convert the frequency part to an integer
		...
		# Handle cases where the frequency part is not a valid integer
		logger.warning(f"Could not convert center frequency part to integer in filename: {file_path}")
		return None
	except Exception as e:
...
def get_drone_info(file_path: str) -> Dict[str, Optional[Union[str, int]]]:
	"""
...
def save_samples_from_recording(
	...
	Args:
		...
		drone_info: A dictionary containing metadata about the drone recording
					...
					'center_freq' (int): Center frequency for spectrogram.
	...
	for i in range(0, signal.size - sample_size + 1, window_size):
		...
		file_name_base = os.path.join(save_path, f'{file_pattern}_{current_index}')
		...
		try:
			...
			stft_matrix = get_stft_spectrogram(spectrogram_input, window_size=fft_size, overlap=128)
			...
			if snr_db is not None:
				...
				stft_matrix_noise = get_stft_spectrogram(gaussian_noise, window_size=fft_size, overlap=128)
				spectrum_noise = get_power_spectrogram_db(stft_matrix_noise)
...
def make_dataset(
    ...
    for i, dat_file in tqdm(enumerate(dat_files), total=total_iter, desc="Processing files", unit="file"):
        try:
            ...
            drone_info = get_drone_info(dat_file)
...
def main(snr_range,
         dataset_path='../../data/Related work from Rowan/new_dataset/',
         base_path='../../data/Relate work from AeroDefense/Data/Raw data/SR20M_G50_cage_RT15/',
         patterns=['*.dat', os.path.join("*", "*.dat")],
...
if __name__ == "__main__":
	...
	main(snr_range, add_original=False)  # Set add_original=True to include the original data without noise
# %%
